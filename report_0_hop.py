...
def prepare_0_hop_report(savepath, site_save_name, time_stamp):
    ...
    worksheet.write_string(row, col + 3, "Cyberlocker_link", title_format)
    row += 1
    ...
    with io.open('./debug/' + site_save_name + "/" + time_stamp + "/links.txt", "r", encoding="utf-8") as f:
        for line in f:
            ...
            worksheet.write_string(row, col + 3, cyberlocker_link, cyberlocker_format)
            row += 1
